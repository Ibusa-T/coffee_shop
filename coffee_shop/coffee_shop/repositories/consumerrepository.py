...
class ConsumerRepository(Baserepository):
    ...
    def findByGender(cls,gender):
        ...
        return cls.master_factory.get_model(cls.name).objects.filter(gender=gender)
    
    @classmethod
    def findByConsumerName(consumerName):
        return cls.master_factory.get_model(cls.name).objects.filter(consumerName=consumerName)
